...
public class GridUI {
    ...
    private final Color color_marked = new Color(238, 233, 233);
    ...
    public boolean isMarked(int i, int j) {
        if ((i >= 0) && (i < Constants.GRID_SIZE) && (j >= 0) && (j < Constants.GRID_SIZE)) {
            return textfields[i][j].getBackground() == color_marked;
        }
        return false;
    }
        ...
        for (int i = 0; i < Constants.GRID_SIZE; ++i) {
            for (int j = 0; j < Constants.GRID_SIZE; ++j) {
                if (isFilled(i, j) && !filledEditable) {
                    get(i, j).setBackground(color_marked);
                    editable(i, j, filledEditable);
                } else {
                    get(i, j).setBackground(color_normal);
